 ...
 * Extends Feature 4 catalogue tests (do not duplicate `it` names).
...
describe("Feature 5 — Ingredients Management", () => {
  ...
  describe("US-5.1 — Add a catalogue ingredient", () => {
    ...
    it("User creates an ingredient with a name that is too long", async () => {
      ...
      expect(alert.text()).toContain("Ingredient name must be 100 characters or fewer.");
      wrapper.unmount();
    });
    ...
    it("User creates an ingredient with a non-numeric price per unit", async () => {
      ...
      const alert = wrapper.find(".v-alert");
      expect(alert.exists()).toBe(true);
      expect(alert.text()).toContain("Ingredient price per unit must be a number.");
      wrapper.unmount();
    });
  });
